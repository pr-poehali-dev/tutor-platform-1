...
def handle_tick(conn) -> dict:
    ...
    ВАЖНО: «занимаем» сегодняшний день только когда уже наступило окно
    публикации (с POST_HOUR_FROM МСК). Иначе ночной/утренний заход на сайт
    (например в 00:08 МСК) пометил бы день как отработанный, а реальный
    пост в 9:00 уже не вышел бы. Теперь ранние заходы день не занимают —
    публикация гарантированно происходит при первом заходе после 9:00 МСК."""
    now_msk = datetime.now(MSK)
    today_msk = now_msk.date()

    # До начала окна публикаций ничего не делаем и день НЕ занимаем — ждём 9:00 МСК.
