...
def humanize(text: str) -> str:
    ...
    t = (text or '').strip()
    t = re.sub(r'\s+', ' ', t)
    # Длинная пауза между предложениями (точка/?/! + заглавная)
    t = re.sub(r'([.!?])\s+([А-ЯA-Z])', r'\1 sil<[700]> \2', t)
    # Средняя пауза после двоеточия и тире
    t = re.sub(r'([:—–])\s+', r'\1 sil<[400]> ', t)
    # Короткая пауза после запятой (помогает с интонацией)
    t = re.sub(r',\s+', ', sil<[250]> ', t)
    # Хвостовая пауза — чтобы фраза не «обрывалась» на последнем слове
    t = t.rstrip('.!?,;') + '. sil<[500]>'
    return t
