...
VOICE = 'filipp'           # бодрый молодой мужской — выбран пользователем
EMOTION = 'good'           # энергично
SPEED = 1.05               # чуть быстрее обычного — для соцсетей
LANG = 'ru-RU'
FORMAT = 'mp3'
SAMPLE_RATE = '48000'
...
def humanize(text: str) -> str:
    """Добавляет паузы — чтобы голос звучал живо."""
    t = (text or '').strip()
    t = re.sub(r'\s+', ' ', t)
    # длинные предложения разбиваем многоточием
    t = re.sub(r'([.!?])\s+([А-ЯA-Z])', r'\1 ... \2', t)
    return t
