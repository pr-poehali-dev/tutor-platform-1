...
SUBJECT_TOPICS = {
    ...
    'personalbrand': {
        'name': 'Личный бренд и влияние',
        'topics': {
            '5-9': ['Основы личного бренда', 'Аудит присутствия', 'Контент-стратегия', 'Сторителлинг эксперта', 'Контент-план', 'Работа с аудиторией и репутацией', 'Нейросети для бренда'],
            '10-11': ['Позиционирование эксперта', 'Аудит и стратегия присутствия', 'Контент-стратегия и работа со СМИ', 'Сторителлинг для экспертов', 'Контент-план и дисциплина', 'Репутация и работа с негативом', 'GPT-ассистент и развитие бренда'],
            'ege': ['Стратегия личного бренда', 'Экспертное позиционирование', 'Контент, который продаёт', 'Сторителлинг и упаковка экспертизы', 'Управление репутацией и негативом', 'Нейросети для контента', 'План развития бренда'],
        }
    },
    'chemistry': {
        'name': 'Химия',
    ...
    'robotics': {
        'name': 'Робототехника',
        'topics': {
            '5-9': ['Основы электроники', 'Arduino и C++', 'Датчики', 'Моторы и сервоприводы', 'Сборка робота', 'Raspberry Pi и Python', 'Компьютерное зрение'],
            '10-11': ['Микроконтроллеры', 'Схемотехника', 'Алгоритмы управления', 'Автономные роботы', 'Raspberry Pi и Python', 'Компьютерное зрение и ИИ', 'Инженерный проект'],
            'ege': ['Электроника', 'Программирование микроконтроллеров', 'Датчики и приводы', 'Алгоритмы робота', 'Компьютерное зрение', 'Проектирование робота'],
        }
    },
    'smartmach': {
        'name': 'Цифровизация производства СмартМаш',
        'topics': {
            '5-9': ['Цифровое производство', 'Цифровая модель изделия', 'Конструкторская документация', 'Инженерный анализ CAE', 'CAM и ЧПУ', 'MES в цеху'],
            '10-11': ['Сквозная интеграция КД-CAE-CAM-MES', 'Цифровой паспорт детали', 'Валидация данных', 'Управляющие программы ЧПУ', 'Трассируемость изменений', 'Внедрение и импортозамещение'],
            'ege': ['Единая цифровая модель', 'КД и цифровой паспорт', 'CAE-анализ', 'CAM и ЧПУ', 'MES-управление', 'Трассируемость и внедрение'],
        }
    },
    'psychology': {
        'name': 'Психология и коучинг',
        'topics': {
            '5-9': ['Основы НЛП и раппорт', 'Метамодель языка', 'Работа с убеждениями', 'Техники КПТ', 'Эриксоновский гипноз', 'Коучинг по модели GROW', 'Переговоры и влияние', 'Первичная диагностика', 'Работа с тревогой и депрессией', 'Этика и границы практики'],
            '10-11': ['Основы НЛП и раппорт', 'Метамодель языка', 'Работа с убеждениями', 'Техники КПТ', 'Эриксоновский гипноз', 'Коучинг по модели GROW', 'Переговоры и влияние', 'Первичная диагностика', 'Работа с тревогой и депрессией', 'Этика и границы практики'],
            'ege': ['Основы НЛП и раппорт', 'Метамодель языка', 'Работа с убеждениями', 'Техники КПТ', 'Эриксоновский гипноз', 'Коучинг по модели GROW', 'Переговоры и влияние', 'Первичная диагностика', 'Работа с тревогой и депрессией', 'Этика и границы практики'],
        }
    },
}
...
# Предметные ориентиры реальных заданий ЕГЭ/ОГЭ (по кодификаторам ФИПИ).
# Помогают ИИ давать задачи в формате настоящего экзамена, а не абстрактные.
EXAM_SUBJECT_HINTS = {
    'Математика': {
        'ege': "Ориентируйся на реальные задания профильного ЕГЭ: простейшие уравнения (№1-2), "
               "планиметрия и стереометрия (№3,8), вычисления и преобразования (№4-7), "
               "теория вероятностей (№9-10), производная и наибольшее/наименьшее значение (№11-12), "
               "тригонометрические и показательные уравнения с отбором корней (№13), текстовые задачи (№9), "
               "задачи с параметром (№17). Ответы — конкретные числа или корни.",
        'oge': "Ориентируйся на реальные задания ОГЭ по математике: числа и вычисления (№1-5 по тексту/рисунку), "
               "алгебра (уравнения, неравенства, графики, прогрессии), геометрия (углы, треугольники, окружности, "
               "площади), практико-ориентированные задачи (квартира, шины, тарифы). Ответы — конкретные числа.",
    },
    'Физика': {
        'ege': "Ориентируйся на реальные задания ЕГЭ по физике: механика (кинематика, динамика, законы сохранения), "
               "МКТ и термодинамика, электродинамика, оптика, квантовая физика. Используй СИ, реалистичные значения, "
               "формулы из кодификатора. Расчётные задачи доводи до числа с единицами.",
        'oge': "Ориентируйся на реальные задания ОГЭ по физике: измерения и единицы СИ, механика, тепловые явления, "
               "электричество, оптика. Расчётные задачи с реальными приборами и значениями, ответ — число с единицами.",
    },
    'Химия': {
        'ege': "Ориентируйся на реальные задания ЕГЭ по химии: строение атома и периодический закон, химическая связь, "
               "классы веществ и их свойства, реакции (ОВР, ионный обмен, электролиз), расчёты по уравнениям "
               "(количество вещества, массовая доля, объём газа). Используй корректные формулы и валентности.",
        'oge': "Ориентируйся на реальные задания ОГЭ по химии: знаки элементов, валентность, типы реакций, свойства "
               "кислот/оснований/солей, расчёты массы и массовой доли. Уравнения должны быть верно расставлены.",
    },
    'Информатика и программирование': {
        'ege': "Ориентируйся на реальные задания ЕГЭ по информатике: системы счисления, логика и таблицы истинности, "
               "анализ алгоритмов, кодирование информации, обработка числовых последовательностей, основы Python. "
               "Числовые ответы доводи до конца.",
        'oge': "Ориентируйся на реальные задания ОГЭ по информатике: кодирование, системы счисления, логика, "
               "простые алгоритмы и таблицы. Ответы — конкретные числа или строки.",
    },
    'Биология': {
        'ege': "Ориентируйся на реальные задания ЕГЭ по биологии: цитология, генетика (задачи на скрещивание), "
               "эволюция, анатомия и физиология человека, экология. Формулировки и термины — строго научные.",
        'oge': "Ориентируйся на реальные задания ОГЭ по биологии: клетка, растения и животные, организм человека, "
               "экология. Термины и факты должны быть корректными и проверяемыми.",
    },
}


def _exam_format_block(grade, subject_name=''):
    """Доп. инструкция для задач формата реального экзамена (ЕГЭ/ОГЭ).

    Для точных наук и grade 10-11 тоже включаем формат ЕГЭ — там идёт подготовка к экзамену.
    """
    exam_kind = None
    if grade == 'ege':
        exam_kind = 'ege'
        ...
        # 10-11 классы готовятся к ЕГЭ — даём экзаменационный формат
        exam_kind = 'ege'
    ...
    subj_hint = ''
    subj_map = EXAM_SUBJECT_HINTS.get(subject_name)
    if subj_map and subj_map.get(exam_kind):
        subj_hint = "\n- " + subj_map[exam_kind]
    ...
    if exam_kind == 'ege':
        return (
            ...
            "однозначное решение, которое сходится с ответом." + subj_hint
        )
    return (
        ...
        "- Каждая задача обязана иметь однозначное решение, которое сходится с ответом." + subj_hint
    )
...
def _generate_lesson_tasks_only(subject_name, topic, grade, difficulty, shown_questions=None, n=5, seed=None):
    ...
    EXACT_SUBJECTS = ('Математика', 'Физика', 'Химия', 'Информатика и программирование')
    is_exam = grade in ('ege', 'oge') or (grade == '10-11' and subject_name in EXACT_SUBJECTS)
...
def build_fallback_lesson(subject, topic, lesson_title=''):
    """Запасной осмысленный урок, когда ИИ временно недоступен.
    Гарантирует, что покупатель курса ВСЕГДА видит полезный контент, а не пустоту
    или ошибку. Особенно важно для взрослых прикладных курсов (личный бренд, контент)."""
    title = lesson_title or topic or 'Урок'
    subject_name = SUBJECT_TOPICS.get(subject, {}).get('name', 'Курс')
    return {
        'title': title,
        'subtitle': f'Разбираем тему «{topic}» по шагам и сразу применяем на практике',
        'duration_minutes': 25,
        'objectives': [
            f'Понять суть темы «{topic}» и зачем она нужна на практике',
            'Разобрать ключевые принципы на конкретных примерах',
            'Получить пошаговый алгоритм, который можно применить сразу',
        ],
        'theory_blocks': [
            {
                'heading': f'Что такое «{topic}» и почему это важно',
                'content': (
                    f'Тема «{topic}» — один из практических блоков курса «{subject_name}». '
                    'Здесь важно не заучивать теорию, а понять логику и применить её к своей ситуации. '
                    'Разберём, из чего складывается результат, какие ошибки мешают и как выстроить рабочую систему шаг за шагом. '
                    'Каждый принцип ниже проверен на практике и даёт понятный эффект.'
                ),
                'key_points': [
                    'Фокус на практике, а не на абстрактной теории',
                    'Работаем на вашем реальном примере',
                    'Каждый шаг ведёт к измеримому результату',
                ],
            },
            {
                'heading': 'Ключевые принципы',
                'content': (
                    f'Чтобы освоить «{topic}», двигайтесь от простого к сложному. '
                    'Сначала разберитесь с основой, затем добавляйте детали и отрабатывайте на своём материале. '
                    'Не гонитесь за объёмом — важнее регулярность и осознанность. '
                    'Возвращайтесь к этому уроку и сверяйтесь с чек-листом ключевых тезисов.'
                ),
                'key_points': [
                    'Основа → детали → практика',
                    'Регулярность важнее объёма',
                    'Сверяйтесь с чек-листом тезисов',
                ],
            },
            {
                'heading': 'Как применить прямо сейчас',
                'content': (
                    f'Возьмите свою реальную задачу и пройдите тему «{topic}» на её примере. '
                    'Выпишите 1–2 конкретных действия, которые сделаете сразу после урока. '
                    'Небольшой, но выполненный шаг ценнее большого плана, который так и остался на бумаге. '
                    'Так вы закрепите навык и увидите первый результат уже сегодня.'
                ),
                'key_points': [
                    'Примените на своей реальной задаче',
                    'Выберите 1–2 действия и сделайте сразу',
                    'Маленький выполненный шаг > большой невыполненный план',
                ],
            },
        ],
        'examples': [
            {
                'title': 'Пример: пошаговое применение',
                'problem': f'Как применить тему «{topic}» к своей задаче за один подход',
                'solution_steps': [
                    'Шаг 1: сформулируйте свою конкретную ситуацию одним предложением',
                    'Шаг 2: определите желаемый результат',
                    'Шаг 3: выберите один принцип из урока, который ближе всего к задаче',
                    'Шаг 4: сделайте первое действие и оцените результат',
                ],
                'answer': 'Готовый первый шаг, применённый к вашей реальной задаче',
                'note': 'Не пытайтесь сделать всё сразу — начните с одного шага.',
            },
        ],
        'common_mistakes': [
            'Учить теорию, но не применять — знание без действия не даёт результата',
            'Пытаться сделать всё идеально сразу — лучше запустить и улучшать по ходу',
            'Бросать на половине — результат приходит с регулярностью',
        ],
        'summary': (
            f'Тема «{topic}» — это прежде всего практика. Примените один принцип к своей реальной задаче '
            'сразу после урока, и вы закрепите навык быстрее, чем при простом чтении теории.'
        ),
        '_fallback': True,
    }
...
{{
  ...
  "theory_blocks": [
    {{
      ...
      "content": "Объяснение в 4-5 предложений. Простым языком, с аналогиями из жизни (пицца, шоколадка, деньги). Конкретные определения, формулы (без LaTeX), правила.",
  ...
  "examples": [
    {{
      ...
      "solution_steps": ["Шаг 1: что делаем","Шаг 2: расчёт","Шаг 3: ответ"],
      "answer": "финальный ответ",
      "note": "важный нюанс (1 предложение)"
    }},
    ... всего 3 разобранных примера от простого к сложному
  ],
...
ЖЁСТКИЕ ТРЕБОВАНИЯ:
- РОВНО 3 раздела теории по 4-5 предложений
- РОВНО 3 разобранных примера со ШАГАМИ
...
- Примеры из жизни (деньги, скорость, спорт, еда)"""
    ...
    # Fallback НЕ кладём в кэш, чтобы позже заменить его настоящим ИИ-уроком.
    if not isinstance(data, dict) or not data.get('theory_blocks'):
        fb = build_fallback_lesson(subject, topic, lesson_title)
        fb['tasks'] = []
        fb['_cached'] = False
        return fb
