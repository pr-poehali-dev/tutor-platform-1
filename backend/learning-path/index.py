...
SUBJECT_TOPICS = {
    ...
    'marketing': {
        ...
        'topics': {
            '5-9': ['Основы интернет-маркетинга', 'Целевая аудитория', 'Контекстная реклама', 'Email-маркетинг', 'Воронка продаж', 'Аналитика', 'Первые клиенты'],
            '10-11': ['Стратегия продвижения', 'Контекстная и таргетированная реклама', 'Email-маркетинг и рассылки', 'Воронки и автоворонки', 'Веб-аналитика и метрики', 'Юнит-экономика рекламы', 'Поиск клиентов и заработок'],
            'ege': ['Performance-маркетинг', 'Управление рекламными кампаниями', 'Email-автоматизация', 'Сквозная аналитика', 'Юнит-экономика', 'Монетизация навыков'],
        }
    },
    'chemistry': {
        'name': 'Химия',
...
✅ КАЖДАЯ ЗАДАЧА ОБЯЗАНА БЫТЬ РЕШАЕМОЙ И НЕ ВЫДУМАННОЙ:
- В условии ДОСТАТОЧНО данных, чтобы получить ровно один однозначный ответ. Никаких недостающих или противоречивых данных.
- Не выдумывай несуществующие факты, формулы, даты, термины. Используй только реальные, проверяемые знания по теме "{topic}".
- В "explanation" приведи ПОЛНОЕ пошаговое решение, которое реально приводит к correct_answer_text. Если решения нет или ответ не сходится — переделай задачу.
{exam_block}
...
def _exam_format_block(grade):
    """Доп. инструкция для задач формата реального экзамена (ЕГЭ/ОГЭ)."""
    if grade == 'ege':
        return (
            "\n\n🎓 ФОРМАТ ЕГЭ — задачи как в реальном экзамене (КИМ ФИПИ):\n"
            "- Формулируй задания строго в стиле и сложности реального ЕГЭ по этому предмету.\n"
            "- Используй типовые формулировки заданий ЕГЭ, реальные единицы и реалистичные числа.\n"
            "- НЕ выдумывай факты: все данные, формулы и значения должны быть корректными и проверяемыми.\n"
            "- ⚠️ ВАРИАНТЫ ОТВЕТА — ДО КОНЦА ВЫЧИСЛЕННЫЕ значения (число, дробь или простое выражение), "
            "а НЕ сырые невычисленные формулы. Например, если получается sin(π)·e^π + cos(π)·e^π, "
            "доведи до конца: sin(π)=0, cos(π)=−1, значит ответ = −e^π. В options пиши именно −e^π, а не "
            "«e^π + sin(π)». Подставь известные значения (sin, cos, корни, логарифмы) и упрости полностью.\n"
            "- Перед ответом РЕШИ задачу до числа и перепроверь подстановкой. Каждая задача обязана иметь "
            "однозначное решение, которое сходится с ответом."
        )
    if grade == 'oge':
        return (
            "\n\n🎓 ФОРМАТ ОГЭ — задачи как в реальном экзамене 9 класса (КИМ ФИПИ):\n"
            "- Формулируй задания строго в стиле и сложности реального ОГЭ по этому предмету.\n"
            "- Используй типовые формулировки заданий ОГЭ, реальные единицы и реалистичные числа.\n"
            "- НЕ выдумывай факты: все данные, формулы и значения корректные и проверяемые.\n"
            "- Каждая задача обязана иметь однозначное решение, которое сходится с ответом."
        )
    return ''
