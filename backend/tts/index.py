...
def make_singing_text(text: str) -> str:
    ...
    произношение в Yandex TTS («Ладушкии» вместо «Ладушки»). Мягкость и
    мелодичность достигаются:
      1. Замедленным темпом голоса (speed в VOICE_MAP) + светлым тоном.
      2. Разными по длине музыкальными паузами (Yandex-разметка sil<[мс]>),
         чтобы каждая строчка «допевалась» — это даёт песенный ритм,
         а не монотонное чтение.
      3. Плавной фразировкой на запятых и мягким затуханием между куплетами.
    """
    if not text:
        return text

    # Многоточие — единая длинная распевная пауза (обрабатываем ДО точек).
    result = text.replace('…', ' sil<[500]> ').replace('...', ' sil<[500]> ')

    # Пустая строка (между куплетами) — самая длинная «музыкальная» пауза.
    result = result.replace('\n\n', ' sil<[900]> ')
    # Конец строки песни — распевная пауза, строчка мягко «допевается».
    result = result.replace('\n', ' sil<[500]> ')

    # После восклицаний/вопросов — лёгкий распев (пауза + подъём интонации).
    result = result.replace('!', '! sil<[350]>').replace('?', '? sil<[350]>')
    # Точка в песне — мягкое «допевание» фразы (короче, чем конец строки).
    result = result.replace('.', '. sil<[350]>')
    # Запятые в песне держим дольше — плавная, напевная фразировка.
    result = result.replace(',', ', sil<[260]>')
    # Тире/дефис между словами — микро-люфт, добавляет напевности, а не рубит фразу.
    result = result.replace(' — ', ' sil<[200]> ')

    # Убираем лишние пробелы, не трогая разметку пауз.
...
VOICE_MAP = {
    ...
    # Няня Лиса поёт нараспев. Голос делаем светлым, мягким и певучим:
    # приподнятый тон убирает низкое «грубоватое» звучание, замедленный темп
    # и распевные паузы дают мелодичность, близкую к живому женскому вокалу.
    'fox_song': {
        'voice': 'alena',
        'role': 'friendly',
        'speed': '0.88',      # плавнее и напевнее, но без «сонной» тягучести
        'pitch_shift': 0,
        'child_pitch': 1.5,   # лёгкий тёплый подъём — живой голос, без «писклявого робота»
    },
    # Колыбельная — самый нежный режим: медленно, тихо, тепло.
    # Мягкий подъём тона делает голос убаюкивающим и бархатным, без басовитости.
    'fox_lullaby': {
        'voice': 'alena',
        'role': 'friendly',
        'speed': '0.84',
        'pitch_shift': 0,
        'child_pitch': 1,     # почти натуральный тёплый женский тембр
    },
}
