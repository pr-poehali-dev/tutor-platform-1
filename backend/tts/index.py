...
def make_singing_text(text: str) -> str:
    ...
    result = result.replace('!', '! sil<[400]>').replace('?', '? sil<[400]>')
    # Запятые в песне держим дольше — плавная, напевная фразировка.
    result = result.replace(',', ', sil<[300]>')
...
VOICE_MAP = {
    ...
    'fox_song': {
        'voice': 'alena',
        'role': 'friendly',
        'speed': '0.86',      # плавнее и напевнее — строчка «допевается»
        'pitch_shift': 0,
        'child_pitch': 3,     # светлый подъём тона — мягкий, звонкий тембр
    },
    ...
    'fox_lullaby': {
        ...
        'speed': '0.82',
        'pitch_shift': 0,
        'child_pitch': 2,     # нежность и мягкость без «детскости»
    },
}
