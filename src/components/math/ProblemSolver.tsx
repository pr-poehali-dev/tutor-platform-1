...
import { Problem } from "@/components/math/problemsData";
...
const DIFFICULTY_STYLE: Record<Problem["difficulty"], { label: string; cls: string }> = {
...
export default function ProblemSolver({ problem, onSolved, solved = false }: Props) {
  ...
  return (
    <div className="bg-card border border-white/10 rounded-3xl overflow-hidden">
      ...
      <div className="p-5 md:p-6 border-b border-white/8">
        ...
        <div className="bg-gradient-to-br from-purple-500/10 to-cyan-500/10 border border-purple-500/25 rounded-2xl p-4">
        ...
        {problem.steps.slice(0, revealedSteps).map((s, i) => (
          ...
          >
            <p className="font-montserrat font-bold text-white text-sm mb-2 flex items-center gap-2">
              <span className="w-6 h-6 rounded-lg bg-gradient-to-br from-purple-500 to-cyan-500 flex items-center justify-center text-white text-xs font-black">
                {i + 1}
              </span>
              {s.title}
            </p>
            <p className="text-white/75 text-sm leading-relaxed mb-2.5">{s.text}</p>
            {s.formula && (
      ...
      <div className="p-5 md:p-6 pt-3 flex flex-wrap gap-2.5">
        {!allRevealed ? (
          <>
            <button
              onClick={handleNext}
              className="flex-1 inline-flex items-center justify-center gap-2 bg-gradient-to-r from-purple-500 to-cyan-500 text-white text-sm font-bold px-5 py-3 rounded-2xl hover:scale-[1.01] transition-transform"
            >
      ...
      {(allRevealed || showAnswer) && (
        <>
          <div className="mx-5 md:mx-6 mb-4 bg-gradient-to-br from-emerald-500/15 to-teal-500/15 border border-emerald-500/35 rounded-2xl p-4">
            ...
            <p className="font-montserrat font-black text-white text-lg md:text-xl">
              {problem.answer}
            </p>
          </div>
          <div className="mx-5 md:mx-6 mb-6 bg-gradient-to-br from-purple-500/10 to-cyan-500/10 border border-purple-500/30 rounded-2xl p-4 flex items-start gap-3">
