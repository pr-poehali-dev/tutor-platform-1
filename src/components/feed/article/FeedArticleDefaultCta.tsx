...
function pickOffer(article: FeedArticle): Offer {
  ...
  if (
    ...
    // Ведём в сам конструктор, а не на страницу условий: там человек за минуту
    // получает готовую программу и видит пользу сам. Раньше ссылка шла
    // на /repetitoram — это рассказ о тарифах до того, как продукт попробовали.
    return {
      emoji: "🏫",
      title: "У вас есть знания и ученики — не хватает только упаковки?",
      text: "Назовите тему — ИИ соберёт программу курса с уроками, заданиями и тестами за минуту. Бесплатно и без регистрации, результат скачивается в PDF.",
  ...
  // Найм и управление исполнителями → Оркестратор. Проверяем до кадровой ветки:
  // «онбординг» и «адаптация» — про введение в работу, а не про документы.
  if (has("онбординг", "адаптация сотрудник", "адаптация нового", "фрилансер", "подрядчик", "исполнител", "делегирован", "техническое задание", "удалённая команда", "удаленная команда")) {
    return {
      emoji: "🎼",
      title: "Соберите трек адаптации исполнителя за полминуты",
      text: "ИИ составит матрицу навыков, входной контроль, онбординг по дням и задачи с критериями «готово». Генерация бесплатна.",
      button: "Собрать трек бесплатно",
      to: "/orchestrator",
      goal: "article_cta_orchestrator",
    };
  }
