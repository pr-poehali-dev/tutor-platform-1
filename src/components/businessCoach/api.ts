...
const URL = (func2url as Record<string, string>)["business-coach"];
...
export async function generatePlan(goal: string, answers: Answers): Promise<GeneratePlanResult> {
  try {
    ...
    if (!res.ok) return { ok: false, message: data.error || "Не удалось собрать программу" };
...
// ── Личный кабинет: сохранение стратегии, прогресс, дневник-коуч ──
...
export const COACH_COURSE_ID = 9201;
