...
import { submitLead, collectUtm, FinReport, Answers } from "./api";
...
export default function LeadForm({ goal, answers, plan, price, onClose }: Props) {
  ...
  const submit = async () => {
    ...
    trackGoal("lead_form_success", { form_type: "fin_advisor" });
    setDone(true);
  };
    ...
    return (
      <div className="rounded-3xl border border-emerald-500/30 bg-emerald-500/[0.08] p-8 text-center">
        ...
        <p className="text-white/70 text-sm">
          Мы изучим ваши цифры по анализу «{plan.report_title}» и свяжемся с вами в течение рабочего дня.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
      <div className="flex items-start justify-between gap-4 mb-1">
        <h3 className="font-montserrat font-black text-xl text-white">Заявка на финансовое сопровождение</h3>
        <button onClick={onClose} className="text-white/40 hover:text-white flex-shrink-0" aria-label="Закрыть">
          <Icon name="X" size={20} />
        </button>
      </div>
      <p className="text-white/55 text-sm mb-5">
        Оставьте контакт — разберём ваш анализ детально и составим план финансового оздоровления.
      </p>
      ...
      <button
        onClick={submit}
        disabled={loading}
        className="mt-4 w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-bold py-3.5 rounded-xl disabled:opacity-60 hover:scale-[1.01] transition-transform"
      >
