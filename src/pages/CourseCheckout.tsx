...
export default function CourseCheckout() {
  ...
  const { canAccessCourse, hasSubscription, buyCourse, confirmDemoPurchase } = useAccess();
  ...
  const handleCreatePurchase = async () => {
    setError(null);
    setProcessing(true);
    const res = await buyCourse(course.id, course.grade, course.title);
    setProcessing(false);
    if (!res.ok) {
                  ...
                  <div className="space-y-3">
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 text-amber-200 text-sm">
                      ...
                      <p className="text-amber-200/80 text-xs">
                        Подключение онлайн-оплаты настраивается. Пока ты можешь активировать доступ кнопкой ниже — мы выдадим курс сразу.
                      </p>
                    </div>
                    <button
