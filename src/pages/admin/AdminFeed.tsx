...
export default function AdminFeed() {
  ...
  const refreshAll = async () => {
    ...
    const [p, s] = await Promise.all([fetchPending(), fetchSources()]);
    setPending(p);
    setSources(s);
        ...
        <section className="bg-gradient-to-br from-cyan-500/15 to-purple-500/15 border border-cyan-500/30 rounded-3xl p-5 md:p-6">
          <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
            ...
            >
              ...
              Запустить обход всех источников
            </button>
          </div>
        ...
        <section className="bg-card border border-white/10 rounded-3xl p-5 md:p-6">
          <div className="flex items-center justify-between gap-2 mb-4 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-amber-300 text-[11px] uppercase tracking-wider font-bold">Модерация</span>
            </div>
            <span className="text-white/55 text-xs">На рассмотрении: <span className="text-white font-bold">{pending.length}</span></span>
          </div>
          <h2 className="font-montserrat font-black text-white text-xl md:text-2xl mb-4">Статьи пользователей</h2>
